...
export class Basemap extends WebMapPlugin {
    ...
    _initTdt() {
        const createTianDiTuItem = (name, proj) => {
            this.createBasemap(`天地图${name}${proj}`, createLayer({ lyrType: 'WebTileLayer', urlTemplate: Basemap[`_tdt${proj}`][`${name}底图`] }));
    ...
    _initCustomItems() {
        this._options.customItems.forEach(item => {
            this.createBasemap(item.key, item.lyrs.map(lyr => createLayer(lyr) // eslint-disable-line @typescript-eslint/no-explicit-any
            ));
        });
        return this;
    }
